...
test('空窝 + 纸条 + 一个把它哄回来的按钮', () => {
  render(<RunawayScreen name="臭宝" note="走了。别找。（找的话，往老地方。）" onCoax={() => {}} busy={false} />)
...
test('没纸条也不炸', () => {
  render(<RunawayScreen name="臭宝" note={null} onCoax={() => {}} busy={false} />)
  expect(screen.queryByText('它留了张纸条')).not.toBeInTheDocument()
})

test('点按钮去哄', async () => {
  const onCoax = vi.fn()
  render(<RunawayScreen name="臭宝" note="哼。" onCoax={onCoax} busy={false} />)
  await userEvent.click(screen.getByRole('button', { name: '🥺 去哄它回来' }))
  expect(onCoax).toHaveBeenCalledOnce()
})

test('哄的过程中按钮禁用，防连点', () => {
  render(<RunawayScreen name="臭宝" note="哼。" onCoax={() => {}} busy />)
