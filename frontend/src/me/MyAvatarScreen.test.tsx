...
/** 造型收在底部抽屉里：点开分身 → 挑一个（挑完自动收起） */
async function pickEmoji(current: string, next: string) {
  await userEvent.click(await screen.findByLabelText(`换造型，当前 ${current}`))
  await userEvent.click(await screen.findByLabelText(`emoji-${next}`))
}

test('28 个 emoji 收在抽屉里，不占页面', async () => {
  mountable()
  renderWithProviders(<MyAvatarScreen onLogout={() => {}} />)
  await screen.findByLabelText('换造型，当前 🐷')
  expect(screen.queryByLabelText('emoji-🦊')).not.toBeInTheDocument()
})

test('当前造型在抽屉里高亮', async () => {
  mountable()
  renderWithProviders(<MyAvatarScreen onLogout={() => {}} />)
  await userEvent.click(await screen.findByLabelText('换造型，当前 🐷'))
  expect(await screen.findByLabelText('emoji-🐷')).toHaveAttribute('aria-pressed', 'true')
...
test('挑完造型只改本地状态，得按「保存」才落库', async () => {
  mountable()
  const bodies = captureSave()
  renderWithProviders(<MyAvatarScreen onLogout={() => {}} />)
  await pickEmoji('🐷', '🦊')

  expect(bodies).toHaveLength(0) // 抽屉关了，但还没提交
  expect(screen.getByLabelText('换造型，当前 🦊')).toBeInTheDocument()
  expect(screen.getByRole('button', { name: '保存' })).toBeEnabled() // dirty 了
})
...
test('老数据没存过 emoji 时，回落到 👾', async () => {
  ...
  renderWithProviders(<MyAvatarScreen onLogout={() => {}} />)
  // 等名字落地才算数据到了——👾 是 useState 的初值，一上来就显示，等它等于没等
